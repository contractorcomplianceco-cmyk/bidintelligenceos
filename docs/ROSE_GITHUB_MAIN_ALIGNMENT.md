...
2. **Aligned `main` with the production line** via force-with-lease (user-approved for unrelated histories):
   ...
   - Current `main` tip: **`7143e5a`** — leadership/contact + export-until-reviewed gates
...
> - **Updated `main`** to the BidOS production line (baseline `6ff2c75`; current tip **`7143e5a`**). **`main` is now the production branch.**
...
main                                        → 6f92d44  (BidOS production line)
feat/bidos-production-2026-07               → 6f92d44  (aligned with main)
```
