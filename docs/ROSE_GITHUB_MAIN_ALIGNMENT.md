...
2. **Aligned `main` with the production line** via force-with-lease (user-approved for unrelated histories):
   ...
   - Current `main` tip: **`ea28c8c`** — alignment doc sync (VoiceConnect stub at `afe436e`; PDF export gate at `7dcfc57`; CI feat-branch gates at `2b1c3e2`)
...
> - **Updated `main`** to the BidOS production line (baseline `6ff2c75`; current tip **`ea28c8c`**). **`main` is now the production branch.**
...
main                                        → ea28c8c  (BidOS production line)
feat/bidos-production-2026-07               → ea28c8c  (aligned with main)
```
