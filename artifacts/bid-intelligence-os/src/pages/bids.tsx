      ...
      <div className="space-y-8 max-w-[1600px] mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 mt-2 text-lg">Manage active pipeline and historical bid data.</p>
          </div>
              ...
              <Table>
                <TableHeader className="bg-slate-950/50">
                  <TableRow className="border-slate-800 hover:bg-transparent">
                    <TableHead className="text-slate-400 font-semibold uppercase tracking-wider text-xs h-12">Opportunity</TableHead>
                      ...
                      <TableCell>
                        <Badge 
                          ...
                          className={`
                            ...
                            ${bid.status === "Won" ? "bg-teal-950/40 text-teal-400 border-teal-900/50" : ""}
                            ${bid.status === "Lost" ? "bg-red-950/40 text-red-400 border-red-900/50" : ""}
                            ${bid.status === "In Progress" || bid.status === "Review" ? "bg-blue-950/40 text-blue-400 border-blue-900/50" : ""}
                            ${!["Won", "Lost", "In Progress", "Review"].includes(bid.status) ? "bg-slate-800/50 text-slate-300 border-slate-700/50" : ""}
                          `}
                        >
                          {bid.status}
                        </Badge>
